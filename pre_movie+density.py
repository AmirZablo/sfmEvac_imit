...
if(len(params)==1):
    ...
    r=float(datos[6][1])
    t=float(datos[7][1])
    verts=((-D/2,0),(-L/2,0),(-L/2,L),(L/2,L),(L/2,0),(D/2,0)) #(x,y)
    ...
    hist=ax[1].hist2d(x,y,bins=[[i for i in range(int(-L/2),int(L/2 +1))],[i for i in range(int(L+1))]],cmap=my_cmap,vmin=0,vmax=9.999)
