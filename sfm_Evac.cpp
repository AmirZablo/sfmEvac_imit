...
int L; //Walls' length (with L=50, you can fit at most 1000 pedestrians in the room)
float D; //Exit's width 
...
pto2D Rotate(pto2D vec, float theta)
{
    pto2D rot;
...
void saveFrame(bool error)
{
    ...
    if(!error)
    {
        ...
        out<<"name x y attitude L D r t\n";
        for(int p=0;p<peds.size();p++)
        {
            out<<to_string(peds[p].name)+" "+to_string(peds[p].x)+" "+to_string(peds[p].y)+" "+peds[p].att+" "+to_string(L)+" "+to_string(D)+" "+to_string(r)+" "+to_string(t)+'\n';
        }
    }
    else
    {
...
pto2D F0(float vx, float vy, float desiredVel, float dirx, float diry, float tau, float mass)
{
    pto2D F;
...
pto2D F_inter(float x1, float y1, float x2, float y2, float r1, float r2, float B, float A)
{
    pto2D F;
...
pto2D F_roz(float dist, Pedestrian p1, Pedestrian p2, float r1, float r2)
{
    pto2D norm;
...
bool isInFOV(float dirx, float diry,float fx,float fy)
{
    float prod=dirx*fx+diry*fy;
    return(prod>=sqrt(pow(fx,2)+pow(fy,2))*cos(phi));
}

void loadParams(string ifile)
{
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        stringstream subdir;
        subdir<<"mkdir sim_"<<folname<<"/frames";
        ...
        for(int i=0; i<NEgo+NCoop; i++)
        {
            ...
            while(trying)
            {
                ...
                y_cand=Randomfloat(r,L-r);
                trying=false;
                for(Pedestrian &p: peds)
                {
                    if((distance(x_cand,y_cand,p.x,p.y)<(4*r)) || (distance(x_cand,y_cand,-L/2,y_cand)<(3*r)) || (distance(x_cand,y_cand,L/2,y_cand)<(3*r)) || (distance(x_cand,y_cand,x_cand,L)<(3*r)) || (distance(x_cand,y_cand,x_cand,0)<(3*r)))
                    {
                        trying=true;
                        break;
                    }
        ...
        while(pedsLeft>0.2*(NEgo+NCoop)) //End simulation when 80% left the room 
        {
            int p_in_r=0;
            for(int p=0; p<peds.size();)
            {
                peds[p].update_e();
                ...
                {
                    if(distance(peds[p].x,peds[p].y,0,0)<r_dens)
                    {
                        p_in_r++;
                    }
                    p++;
            ...
            for(int p=0;p<peds.size();p++)
            {
                ...
                peds[p].new_ax+=F_inter(peds[p].x,peds[p].y,-L/2,peds[p].y,r,0,peds[p].B,peds[p].A).x/m;
                peds[p].new_ay+=F_inter(peds[p].x,peds[p].y,-L/2,peds[p].y,r,0,peds[p].B,peds[p].A).y/m;

                peds[p].new_ax+=F_inter(peds[p].x,peds[p].y,L/2,peds[p].y,r,0,peds[p].B,peds[p].A).x/m;
                peds[p].new_ay+=F_inter(peds[p].x,peds[p].y,L/2,peds[p].y,r,0,peds[p].B,peds[p].A).y/m;

                peds[p].new_ax+=F_inter(peds[p].x,peds[p].y,peds[p].x,L,r,0,peds[p].B,peds[p].A).x/m;
                peds[p].new_ay+=F_inter(peds[p].x,peds[p].y,peds[p].x,L,r,0,peds[p].B,peds[p].A).y/m;

                if(peds[p].x<(-D/2) || peds[p].x>(D/2))
                {
                    peds[p].new_ax+=F_inter(peds[p].x,peds[p].y,peds[p].x,0,r,0,peds[p].B,peds[p].A).x/m;
                ...
                else if(peds[p].x>(-D/2) && peds[p].x<(-D/2 + r))
                {
                    peds[p].new_ax+=F_inter(peds[p].x,peds[p].y,-D/2,0,r,0,peds[p].B,peds[p].A).x/m;
                ...
                float interVx=(peds[p].x-peds[p].xPrev)/dt;
                float interVy=(peds[p].y-peds[p].yPrev)/dt;

                peds[p].new_ax+=F0(interVx,interVy,peds[p].desiredVel,peds[p].dirx,peds[p].diry,peds[p].tau,m).x/m;
            ...
            for(int p=0;p<peds.size();p++)
            {
                ...
                //Cota vmax
                if(sqrt(pow(peds[p].vx,2)+pow(peds[p].vy,2))>vmax)
                {
            ...
            dens_exit<<t<<" "<<2*p_in_r/(PI*r_dens*r_dens)<<" "<<r_dens<<endl;
            t=roundf((t+dt) * 1000) / 1000;
            iter+=1;
            if(iter%frameSaveRate==0)
            {
                saveFrame(false);
            }
            if(t>tmax)
